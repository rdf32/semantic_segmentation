...
def tf_train_dataset(x, y, batch, epochs, shape, num_classes):
    """
    ...
    before the data is returned in batches

    :param: x: list of image paths - output out load_data()
    :param: y: list of mask paths - output out load_data()
...
def tf_val_dataset(x, y, batch, epochs, shape, num_classes):
    """
    Creates a tf.data.Dataset from a list of image and mask paths
    the lists of paths are sent to the from_tensor_slices() method and then each pair image/mask pair
    is sent through the preprocess() function which reads them in convertes them to their respective np.arrays
...
def get_dataset(paths, data_shape, batch_size, epochs, num_classes):
    ...
    train_dataset = tf_train_dataset(train_images, train_masks, shape=data_shape, batch=batch_size, epochs=epochs,  num_classes=num_classes)
    
    val_images, val_masks = load_data(val_img_path, val_mask_path)
    print(f"Images: {len(val_images)} - Masks: {len(val_masks)}")
    
    val_dataset = tf_val_dataset(val_images, val_masks, shape=data_shape, batch=batch_size, epochs=epochs,  num_classes=num_classes)
