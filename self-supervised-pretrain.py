...
def run_pretraining(encoder_name, temperature, width, shape, batch_size, epochs, paths, **contrastive_augmentation):
    ...
    encoder_output = paths['output'] + f"/{encoder_name}_encoder.h5"
    callbacks = [
    ...
    data = {"c_loss": history.history['c_loss'], "c_accuracy": history.history['c_acc']}
...
if __name__ == '__main__':
    ...
    run_pretraining('unet', temperature=temperature, width=width, shape=shape, batch_size=batch_size, epochs=epochs,
     paths=paths, **contrastive_augmentation=**contrastive_augmentation)