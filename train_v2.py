...
def get_compiled_model(model_name, set_weights=False, encoder=None):
    ...
    """
    models = {
    unet = unet_model(input_size=(256, 256, 3), n_filters=64, batch_norm=True),
...
def run_training(model_name, paths = {}, data_shape=[], batch_size=32, epochs=30, num_classes=5, set_weights=False, encoder=None):
    """
    ...
    with strategy.scope():
        model = get_compiled_model(model_name=model_name, set_weights=set_weights, encoder=encoder)
    ...
    df.to_csv(paths['output'] + "/" + model_name + "metric_history.csv") # metric save location **
...
if __name__ == '__main__':
    ...
    run_training('unet', paths=paths, data_shape=[(256,256,3),(256,256,5)], batch_size=16, epochs=30, num_classes=5, set_weights=False, encoder=None)
